...
public class Client {
	...
	private static Controller load(String name) {
		...
		try {
			controller = (Controller) Class.forName(name).getDeclaredConstructor().newInstance();
		} catch (ClassNotFoundException e) {
			System.out.println(name + " is not a class name");
			System.exit(0);
		} catch (InstantiationException e) {
			System.out.println("Error instantiating " + name + ": " + e.getMessage());
			System.exit(0);
		} catch (IllegalAccessException e) {
			System.out.println("No access to " + name + " constructor: " + e.getMessage());
			System.exit(0);
		} catch (NoSuchMethodException e) {
			System.out.println("No default constructor found for " + name);
			System.exit(0);
		} catch (java.lang.reflect.InvocationTargetException e) {
			System.out.println("Exception thrown by constructor for " + name);
			System.exit(0);
		}
		return controller;
	}
